...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO users (first_name, last_name, email, password,no_books) VALUES (?, ?, ?, ?,?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssss", $f_name, $l_name, $email, $pass,$no_bo); // "ssss" represents four strings
    $stmt->execute();

    if ($stmt->affected_rows > 0) {
        echo '<meta http-equiv="refresh" content="2; url=/login.php"> <span style="color:green;">New record added successfully. You will redirecting... </span>';
    } else {
...
?>
                    ...
                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                        ...
                        <div class="flex -mx-3">
                            <div class="w-full px-3 mb-5">
                                <button
                                    class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold" type="submit">REGISTER
                                    NOW</button>
                            </div>
                        </div>
                    </form>
